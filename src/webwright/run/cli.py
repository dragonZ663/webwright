...
DEFAULT_CONFIGS = ["base.yaml", "custom_openrouter.yaml"]
DEFAULT_TASK_ID = "bilibili"
DEFAULT_START_URL = "https://www.bilibili.com/"
DEFAUULT_OUTPUT_DIR = "/home/feilong/repositories/"
IF_DEBUG = True
...
def main(
    ...
    task_id: str | None = typer.Option(
        DEFAULT_TASK_ID, "--task-id", help="Optional identifier used in the output directory name."
    ),
    start_url: str | None = typer.Option(
        DEFAULT_START_URL, "--start-url", help="Optional starting URL for the task."
    ),
    config_spec: list[str] = typer.Option(DEFAULT_CONFIGS, "-c", "--config"),
    output_dir: Path | None = typer.Option(DEFAUULT_OUTPUT_DIR, "-o", "--output-dir"),
